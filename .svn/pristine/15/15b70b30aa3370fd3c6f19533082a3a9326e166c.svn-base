...
//		webView.loadUrl("javascript: alert('123')");
	...
	private void checkVersion() {
		UmengUpdateAgent.setUpdateOnlyWifi(false);//是否在WIFI网络下才允许更新
	...
	private void showUpdateDialog(final String downloadUrl, final String message) {
		...
		updateAlertDialog.setIcon(R.mipmap.back);
...
//				System.out.println("lw     onReceivedSslError");
	...
	public void initWeb(String url) {
		...
		otherWebView = (WebView) findViewById(R.id.otherWebView);
		// 设置WebView执行JavaScript脚本
		otherWebView.getSettings().setJavaScriptEnabled(true);
		// Technical settings
		otherWebView.getSettings().setSupportZoom(true); // Scale switch
		otherWebView.getSettings().setSupportMultipleWindows(true); // Multi window
		// 开始应用H5缓存api
		otherWebView.getSettings().setAppCacheEnabled(true);
		// 开启数据库存储API
		otherWebView.getSettings().setDatabaseEnabled(true);
		// 开启 DOMstorage API
		otherWebView.getSettings().setDomStorageEnabled(true);

		/* 缓存 */
		otherWebView.getSettings().setAppCacheMaxSize(1024 * 1024 * 8);// 设置缓冲大小，我设的是8M
		String appCacheDir = this.getApplicationContext()
				.getDir("cache", Context.MODE_PRIVATE).getPath();
		otherWebView.getSettings().setAppCachePath(appCacheDir);
		otherWebView.getSettings().setAllowFileAccess(true);
		// 设置缓冲的模式
		// otherWebView.getSettings().setCacheMode(
		// WebSettings.LOAD_CACHE_ELSE_NETWORK);
		// 支持通过JS打开新窗口
		otherWebView.getSettings().setJavaScriptCanOpenWindowsAutomatically(true);
		otherWebView.setWebViewClient(new WebViewClient() {
			...
			public boolean shouldOverrideUrlLoading(
				...
				WebView.HitTestResult hit = view
						.getHitTestResult();
				if (hit != null) {
...
//				}
		...
		otherWebView
				.setOnKeyListener(new WebView.OnKeyListener() {
